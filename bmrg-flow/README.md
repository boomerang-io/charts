...
The worker priviledged RBAC is dependent on the Pod Security Policy and Role resources;
The charts allows the end-user to configure the name of the security policy depending on the underlying Kubernetes implementation.
For example:
 - running on a OpenShift Container Platform, the name depends on the existence of `securitycontextconstraints` named resource,
 - running on a IBM Cloud Private flavour, the name depends on the existence of `podsecuritypolicies` named resource.
